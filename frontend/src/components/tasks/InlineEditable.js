...
const InlineForm = ({boardId, id, setOpenEdit, field, fieldValue, multi, max}) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!value.trim()) {
      ...
      if (value.trim() !== fieldValue) {
        ...
        dispatch(updateTask({taskId: id, boardId, id, data}))
      }
      setOpenEdit(false)
    }
  }
...
export const TaskNameEdit = ({id, boardId, name, isRequesting}) => {
...
export const TaskDescriptionEdit = ({id, boardId, isRequesting, description}) => {
  const [openEdit, setOpenEdit] = useState(false)
  const handleOpen = () => {
    if (isRequesting)
      return
